...
void sendFile(FILE *fd) {
    ...
    while (current < filesize) {
        ...
        size_t bytesRead = fread(send.data, sizeof(char), 1024, fd);
        ...
        if(poll_result == -1){
            ...
            // Timeout occurred, then resend the packet
            printf("Timeout! Resend!");
            continue;
        }
        else{
            size_t recv_len = recvfrom(sockfd, &recv, sizeof(recv), 0, NULL, NULL);
            if(recv_len == -1){
                perror("recvfrom()");
                exit(EXIT_FAILURE);
            }
